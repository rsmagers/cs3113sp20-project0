...
charInfo add(char byte) 
{
	struct charInfo addMe;
	//TODO fix	addMe.character = byte;
	addMe.count = 1;

	return addMe;
}

int previous(struct charInfo* list, char byte, int spot)
{

	int i = 0;
//	char testMe;
	while(i < spot + 1) {
		//TODO fix		testMe = (byte & list[i].character);
		// if = increment
		//TODO fix		if(byte == testMe) {
		//	list[i].count +=1;
		//	return 1;
		//}
		// if does not exist
...
//void parse(struct charInfo* list, int spot, char byte) 
void parse(int byte)
{
	if((byte & 128) == 128) 
	{
		printf("hey hey now what you doing?\n");
	}
